    ...
    return (
        <section className='flex justify-center'>
            <Form headingText='Account Verification' endpoint='/auth/forgot_password/otp_verify' method='POST' data={data} setError={setError} redirect={'/reset_password'} error={error}>
                <h2 className=' font-serif text-center'>We have sent  OTP code to <span className='text-sm'>{''}</span>.</h2>
                <div className='text-sm font-medium text-neutral-600'> OTP expires in <span className='font-medium'>{ }</span></div>
